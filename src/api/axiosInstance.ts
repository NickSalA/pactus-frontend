...
export const apiGet = async <T>(url: string, config?: Record<string, unknown>): Promise<T> => {
  const response = await axiosInstance.get<T>(url, config as any);
  return response as unknown as T;
};

export const apiPost = async <T>(url: string, data?: unknown, config?: Record<string, unknown>): Promise<T> => {
  const response = await axiosInstance.post<T>(url, data, config as any);
  return response as unknown as T;
};

export const apiPatch = async <T>(url: string, data?: unknown, config?: Record<string, unknown>): Promise<T> => {
  const response = await axiosInstance.patch<T>(url, data, config as any);
  return response as unknown as T;
};

export const apiDelete = async <T>(url: string, config?: Record<string, unknown>): Promise<T> => {
