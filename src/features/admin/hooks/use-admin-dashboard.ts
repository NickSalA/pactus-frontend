...
export type AdminMetricId = "alerts" | "folders" | "services" | "templates" | "users";
export type AdminMetricTone = "amber" | "blue" | "emerald" | "indigo" | "violet";
...
export function useAdminDashboard() {
  ...
  const [summary, setSummary] = useState<AdminSummary>(() => peekAdminCache<AdminSummary>("dashboard-summary") ?? EMPTY_SUMMARY);
  const [loading, setLoading] = useState(() => peekAdminCache<AdminSummary>("dashboard-summary") === null);
  ...
  const loadSummary = useCallback(async (options: { force?: boolean } = {}) => {
    ...
    const cachedSummary = !options.force ? readAdminCache<AdminSummary>("dashboard-summary", ADMIN_CACHE_TTL_MS) : null;
    if (cachedSummary) {
    ...
    try {
      ...
      const [members, rules, templates, services, documents, folders] = await Promise.all([
        getOrganizationMembers(),
        getNotificationRules(),
        getTemplates(),
      ...
      const nextSummary = {
        ...
        activeServiceCount: services.filter((service) => service.is_active).length,
