    ...
    <div className="space-y-8">
      <section className="rounded-[32px] border border-slate-200/70 bg-white px-8 py-7 shadow-sm shadow-slate-200/70">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Configuración de Seguridad</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900">Gestión de Accesos</h1>
              <p className="mt-1 text-sm text-slate-500">Lista usuarios de la organización y agrega nuevos accesos al sistema</p>
            </div>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <article className="rounded-[28px] border border-slate-200/80 bg-white px-6 py-5 shadow-sm shadow-slate-200/70">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Usuarios Activos</p>
          <p className="mt-3 text-4xl font-semibold tracking-tight text-slate-900">{page.stats.activeMembers}</p>
          <p className="mt-2 text-sm text-slate-500">de {page.stats.totalMembers} total</p>
        </article>
        <article className="rounded-[28px] border border-slate-200/80 bg-white px-6 py-5 shadow-sm shadow-slate-200/70">
          <div className="flex items-center gap-3 text-blue-600">
            <Users className="h-5 w-5" />
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Gestión de Usuarios</p>
          </div>
          <p className="mt-3 text-base font-medium text-slate-900">Administra los accesos de tu organización</p>
          <p className="mt-2 text-sm text-slate-500">Puedes listar usuarios y registrar nuevos miembros con el rol correspondiente.</p>
        </article>
      </section>
