...
export function TemplateFormModal({
  ...
  const [generationMode, setGenerationMode] =
    useState<ApiTemplateGenerationMode>('adaptive');
  ...
  const visibleFormats = useMemo(() => {
    return formats.filter(
      (format) =>
        !selectedDocumentType || format.document_type === selectedDocumentType,
    );
  }, [formats, selectedDocumentType]);

  const selectedFormat = useMemo(() => {
    return (
      formats.find((format) => format.format_code === selectedFormatCode) ??
      null
    );
  ...
  useEffect(() => {
    ...
    const initialDocumentType =
      allowedDocumentTypes?.[0] ?? formats[0]?.document_type ?? null;
    const initialFormat =
      formats.find((format) => format.document_type === initialDocumentType) ??
      formats[0] ??
      null;
  ...
  useEffect(() => {
    if (
      ...
      !selectedFormat ||
      selectedFormat.document_type === selectedDocumentType
    ) {
      return;
    }

    const nextFormat =
      formats.find((format) => format.document_type === selectedDocumentType) ??
      null;
  ...
  const navigateToStep = (step: TemplateFormStep) => {
    ...
    window.setTimeout(() => {
      ...
      setSummary2Expanded(false);
      setMaxStepReached(
        (previous) => (step > previous ? step : previous) as TemplateFormStep,
      );
      setVisible(true);
    }, 150);
  };
  ...
  const handleSelectFormat = (formatCode: string) => {
    const nextFormat =
      formats.find((format) => format.format_code === formatCode) ?? null;
    ...
    try {
      ...
      setStepError(
        error instanceof Error
          ? error.message
          : 'No se pudo completar la creación de la plantilla.',
      );
    } finally {
                  ...
                  <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-5 grid gap-4 md:grid-cols-2">
                      {visibleFormats.map((format) => {
                        const isSelected =
                          selectedFormatCode === format.format_code;
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleSelectFormat(format.format_code)
                            }
                ...
                {currentStep === 2 && (
                  <>
                    ...
                    <TemplateSummaryAccordion
                      ...
                      onToggle={() =>
                        setSummary1Expanded((previous) => !previous)
                      }
                    ...
                    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                      <div className="mb-5">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          Puedes personalizar el nombre y la descripción antes
                          de continuar.
                        </p>
                      </div>
                ...
                {currentStep === 3 && (
                  <>
                    ...
                    <TemplateSummaryAccordion
                      ...
                      onToggle={() =>
                        setSummary2Expanded((previous) => !previous)
                      }
                    ...
                    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                      <div className="mb-5">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          Puedes usar instrucciones de texto, un PDF de
                          referencia, o ambos para mejorar el resultado.
                        </p>
                      </div>
                          ...
                          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                            <div className="mb-4">
                              ...
                              <p className="mt-1 text-xs text-slate-500">
                                Controla qué tan literal será el borrador
                                respecto a la referencia entregada.
                              </p>
                            </div>
                              ...
                              <label className="space-y-1.5">
                                ...
                                <Select
                                  ...
                                  onChange={(event) =>
                                    setGenerationMode(
                                      event.target
                                        .value as ApiTemplateGenerationMode,
                                    )
                                  }
                                >
                          ...
                          >
                            {referenceFile ? (
                              ...
                              <>
                                ...
                                <p className="mt-1 text-xs text-slate-500">
                                  Arrastra un archivo aquí o usa el botón para
                                  seleccionarlo.
                                </p>
