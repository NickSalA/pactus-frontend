...
const EMPTY_CONTENT: ApiTemplateContent = {
  body_md: '',
  fields: [],
  operational_fields: [],
  version: '1.0',
...
export function TemplateEditModal({
  ...
  const [
    ...
    setEditorState,
  ] = useState(buildInitialState(template));
  ...
  const detectedFieldKeys = useMemo(
    () => extractTemplateFieldKeys(content.body_md),
    [content.body_md],
  );
  const derivedFields = useMemo(() => {
    const existingMap = new Map(
      getAllTemplateFields(content).map((field) => [field.key, field]),
    );
    return detectedFieldKeys.map((key) => {
      ...
      return existingField
        ? {
            ...existingField,
            type: normalizeTemplateFieldType(existingField.type),
          }
        : {
            key,
            label: formatTemplateFieldLabel(key),
            type: 'text',
            required: false,
          };
    });
  ...
  const visibleFieldKeys = useMemo(
    () => new Set(derivedFields.map((field) => field.key)),
    [derivedFields],
  );
  const operationalFields = useMemo(() => {
    return getTemplateOperationalFields(content).filter(
      (field) => !visibleFieldKeys.has(field.key),
    );
  }, [content, visibleFieldKeys]);

  const totalPages = Math.max(
    1,
    Math.ceil(derivedFields.length / FIELDS_PAGE_SIZE),
  );
  const pagedFields = derivedFields.slice(
    fieldsPage * FIELDS_PAGE_SIZE,
    (fieldsPage + 1) * FIELDS_PAGE_SIZE,
  );
  ...
  const requiredCount = [...derivedFields, ...operationalFields].filter(
  ...
  const validateStep = (step: EditStep): string | null => {
    ...
    if (step === 2 && !content.body_md.trim())
      ...
      const saved = await updateTemplateMutation({
        ...
        payload: {
          ...
          content: {
            ...
            operational_fields: getTemplateOperationalFields(content).filter(
              (field) => !visibleKeys.has(field.key),
            ),
          },
        },
      });
  ...
  const basePreview = (
    <p className="mt-0.5 truncate text-sm text-slate-700">
      ...
      {description.trim() && (
        <span className="text-slate-400"> · {description.trim()}</span>
      )}
    </p>
  );
  ...
  const contentPreview = (
    <p className="mt-0.5 text-sm text-slate-700">
      ...
      <span className="text-slate-400">
        {' '}
        línea{markdownLines === 1 ? '' : 's'} · {content.body_md.length}{' '}
        caracteres
      </span>
    </p>
  );
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-slate-50">
              {['Variable', 'Etiqueta', 'Tipo', 'Placeholder', 'Requerido'].map(
                (header) => (
                  <th
                    key={header}
          ...
          <div className="space-y-6">
            ...
            {isReadonly && (
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                ...
                Esta plantilla está publicada y no puede editarse. Solo las
                plantillas en estado <strong className="mx-1">
                  Borrador
                </strong>{' '}
                son modificables.
              </div>
            ...
            {currentStep >= 2 && (
              ...
              >
                <div className="border-t border-slate-200 bg-white px-4 pb-4 pt-4 text-sm text-slate-600">
                  ...
                  <p className="mt-2">
                    <span className="font-medium text-slate-800">Formato:</span>{' '}
              ...
              <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Edita el cuerpo en Markdown. Los campos se detectan
                      automáticamente en el siguiente paso.
                    </p>
                  </div>
                </div>
                <label className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setContent({ ...content, body_md: e.target.value })
                    }
                    rows={20}
                    disabled={isReadonly}
              ...
              <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="mb-5 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Campos detectados automáticamente a partir del contenido
                        de la plantilla.
                      </p>
                    </div>
                  </div>
                  {(derivedFields.length > 0 ||
                    ...
                    <span className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                      {derivedFields.length + operationalFields.length} campo
                      {derivedFields.length + operationalFields.length === 1
                        ? ''
                        : 's'}
                    </span>
                  )}
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    {renderFieldTable(
                      pagedFields,
                      'No se detectaron campos visibles en el contenido de esta plantilla.',
                    )}

                    {derivedFields.length > 0 && totalPages > 1 && (
                      <div className="mt-3 flex items-center justify-between text-sm text-slate-500">
                        <span>
                          Mostrando {fieldsPage * FIELDS_PAGE_SIZE + 1}–
                          {Math.min(
                            (fieldsPage + 1) * FIELDS_PAGE_SIZE,
                            derivedFields.length,
                          )}{' '}
                          de {derivedFields.length}
                        </span>
                  ...
                  <div>
                    ...
                    {renderFieldTable(
                      operationalFields,
                      'No se definieron campos operativos adicionales en esta plantilla.',
                    )}
                  </div>
                </div>
              </section>
