import type { ApiAuditTemplateActivityResponse } from '@/types/api';
...
import { TEMPLATE_ACTION_LABELS, TEMPLATE_ACTION_COLORS } from '@/features/admin/lib/auditUtils';

type AdminAuditTemplatesTableProps = {
  items: ApiAuditTemplateActivityResponse[];
};

export function AdminAuditTemplatesTable({ items }: AdminAuditTemplatesTableProps) {
  return (
                ...
                <tbody className="divide-y divide-slate-200/80 bg-white text-sm text-slate-700">
                  {items.map((item) => (
                    <tr key={item.id} className="text-sm text-slate-700 hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${TEMPLATE_ACTION_COLORS[item.action]}`}
                        >
                          {TEMPLATE_ACTION_LABELS[item.action]}
                        </span>
                      </td>
