...
const buildDraftFromRule = (rule: NotificationRule | null, fallbackDraft: RuleDraft): RuleDraft => {
  if (!rule) {
...
export function AdminNotificationRuleModal({
  ...
  const [draft, setDraft] = useState<RuleDraft>(buildDraftFromRule(rule, initialDraft));
      ...
      footer={
        <div className="flex items-center justify-end gap-3">
          ...
          >
            ...
            {isSubmitting ? "Guardando..." : rule ? "Guardar cambios" : "Crear alerta"}
          </button>
        </div>
      }
      onClose={onClose}
    ...
    >
      <div className="space-y-5">
        ...
        <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700">Días antes del vencimiento</label>
            <input
              type="number"
              min={1}
