...
import type { CurrencyType, TopRankingSortBy } from "@/types/api.types";

const DASHBOARD_KEY = ["dashboard"] as const;

export const useAreaChartCompany = (currency?: CurrencyType) =>
  useQuery({
    queryKey: [...DASHBOARD_KEY, "area-chart", "company", currency],
    queryFn: () => getAreaChartCompany(currency),
  });

export const useAreaChartLabor = (currency?: CurrencyType) =>
  useQuery({
...
export const useTopCompanies = (currency?: CurrencyType, sort_by?: TopRankingSortBy) =>
  useQuery({
    queryKey: [...DASHBOARD_KEY, "top-companies", currency, sort_by],
    queryFn: () => getTopCompanies(currency, sort_by),
  });

export const useTopServices = (currency?: CurrencyType, sort_by?: TopRankingSortBy) =>
  useQuery({
